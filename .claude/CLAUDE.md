...
2. **Tests:** `npm test` — must be green before deploying. The suite is small and fast (~150 ms); a failure here means the money math is wrong, which is never acceptable to ship.
   ...
   **One** error was suppressed at the site with a reason rather than "fixed": the `react-hooks/refs` report on `useNodesState(buildNodes())` in `CampaignCanvas.tsx`, where `buildNodes()` reads `nodePositionsRef.current` during render. **That ref-read is the fix for the #185 loops**, not a mistake — see docs/canvas-wizard.md before touching it. A second disable on `useEdgesState(buildEdges())` was removed on 2026-08-05: ESLint reported it as an unused directive because `buildEdges()` never touches that ref.
...
- **`snapchat-placement-debugger`** — ad-squad PLACEMENT problems: "Smart / Automatic placement" (`placement_v2`), E2025, and the DPA/CHAT_FEED constraint. Works from LIVE evidence (Vercel logs + the real API response), never docs. **The headline fact, resolved 2026-08-05: E2025 is NOT a lock** — it means the PUT omitted `placement_v2`, and echoing the squad's own value back (read via `?return_placement_v2=true`) makes budget/bid/status all editable. Read the `placement_v2` section of `docs/snapchat-api.md` before touching anything here, especially the reason the echo must be conditional. `/api/debug/placement-probe` no longer exists (deleted 2026-08-04). **TRIGGER** when a campaign won't launch on Smart Placements, E2025 appears again, or `placement_v2` behaviour changes. **SKIP** for generic field compliance (use `snapchat-api-auditor`), security, or wizard/canvas bugs.
...
**Debug Smart Placements (placement_v2 / E2025):**
> Invoke the `snapchat-placement-debugger` agent. Read the `placement_v2` section of `.claude/docs/snapchat-api.md` first — E2025 is a missing-field error, not a lock, and the fix already ships. Do NOT change any placement code from docs or guesses. Reproduce against a throwaway PAUSED squad in a test ad account, scoring every edit on `sub_request_status` AND a fresh-GET read-back (Snapchat returns HTTP 200 with an embedded error, so HTTP status proves nothing), and test BOTH an ordinary squad and a `placement_v2` one before proposing anything — a change that only works for one of the two will corrupt the other.
