...
- **Canvas wizard:** `WizardShell` renders in three modes: `canvas` (`CampaignCanvas` React Flow), `review` (`ReviewAndPost`), `done` (success screen). The done screen displays a `launchError` state (set on any unhandled submission throw, reset by "New Build") so launch failures are visible instead of silently swallowed. `CampaignCanvas` is loaded via `next/dynamic` with `ssr: false`. The canvas uses `useCanvasStore` (Zustand) to track `creativeRows: CreativeRow[]` (each row holds an ordered list of `groupIds`), `creativeGroups: CreativeGroup[]` (each group holds 1–5 asset IDs), four edge lists (`rowToProvider`, `providerToArticle`, `articleToPreset`, `articleToAdAccount`), `nodePositions`, and `routerNodes`. `buildCampaignMatrix()` iterates `rowToProvider` edges → for each row iterates `row.groupIds` → each group produces one `CampaignBuildItem` per article/preset/account combination, with `creativeIds: string[]`. Cascade: removing a row→provider edge that orphans a provider also removes its article edges and any `articleToAdAccount` edges for those orphaned articles; removing a provider→article edge that orphans an article also removes its preset edges. On launch, `WizardShell` loads all assets for `item.creativeIds`, calls `synthesizeCampaign()`, then `runSubmission()`.
  ...
  **Creative row concept:** A `CreativeRow` is the top-level canvas unit — one row node, one shared React Flow handle, connects to one or more providers. Each card slot in the row = one `CreativeGroup` = one ad set. Within a group, multiple creatives = multiple ads in that ad set. Node ID prefix: `row-{id}`. Cards render left-to-right; `groupIds` is prepend-on-add so `groupIds[0]` = newest = leftmost, `groupIds[last]` = oldest = rightmost (closest to handle). "+" button above the rightmost card adds a new slot (prepends). Each time a slot is prepended the node's x position is decreased by `ROW_CARD_STEP = 172` (160 + 12) to keep the rightmost card anchored at the same canvas position. A slot with 2–5 creatives shows a clickable **×N badge** that opens an inline overlay revealing mini-thumbnails of all creatives; each mini has a ✕ to remove that creative. "↓ New row" and "⧉ Duplicate" actions appear on row hover below the row. Max 8 slots per row.
