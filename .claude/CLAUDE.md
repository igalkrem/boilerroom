...
- **Database:** Neon Postgres via `@vercel/postgres` (`POSTGRES_URL` env var) — reporting cache (3 tables: `snapchat_ad_squad_stats`, `kingsroad_report`, `report_sync_log`) + channel lifecycle (`feed_provider_channels`). Migrations run automatically on first call to either `/api/reporting/sync` or `/api/reporting/combined` via `runMigrations()` in `src/lib/db/index.ts`. **Note:** `@vercel/postgres` is deprecated upstream — migrate to `@neondatabase/serverless` when convenient.
...
│   ├── performance/
│   │   ├── PerformanceTable.tsx       # Meta Ads Manager-style table: toolbar (Edit/Delete/Columns/CSV), bulk edit panel, 24 optional metric cols, delivery badge, inline Budget/Bid/Status editing, sort arrows, click name → DrilldownModal; ColumnSelector lives in toolbar
...
│   └── rate-limiter.ts                # rateLimitedCall (token bucket, max 10 req/s) + rateLimitedFetch (wraps rateLimitedCall + 429 retry w/ exponential backoff: 2s/4s/8s/16s, 4 retries). snapFetch uses rateLimitedFetch so all Snapchat API calls get 429 retry automatically.
└── types/
