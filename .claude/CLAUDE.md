...
- **Submission orchestrator:** `lib/submission-orchestrator.ts` runs **five stages** in sequence:
  ...
  2. **Channel assignment** — if `provider.channelConfig.type === "provider-supplied"`, calls `POST /api/feed-providers/channels/assign`; resolves `{{channel.id}}` in all campaign/squad/creative names and in each creative's URL
...
- **Ad squad IDOR protection:** `updateAdSquad` in `src/lib/snapchat/adsquads.ts` accepts `expectedAdAccountId` (required). After fetching the current squad from Snapchat, it asserts `current.ad_account_id === expectedAdAccountId` if the field is present — throwing `"forbidden: ..."` if the squad belongs to a different account. The PATCH route maps this to a 403 response.
