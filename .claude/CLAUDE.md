...
├── components/
│   ├── wizard/
│   │   ├── CampaignCanvas.tsx         # React Flow canvas — row nodes only; no provider/article/account/preset nodes; no edges; dark bg; fitView maxZoom 0.75
...
│   │   ├── nodes/
│   │   │   └── CreativeGroupNode.tsx  # Row node: horizontal row of portrait cards + inline 4-step config panel (Feed/Articles/Accounts/Preset). Provider/article/account/preset pickers open as centered modal checklists from "+ Add" buttons. Headline/CTA editor expands inline below the article pill row. Max 8 card slots per row.
...
└── types/
    ├── wizard.ts                      # CampaignFormData, AdSquadFormData, CreativeFormData, SubmissionResults, CreativeGroup, CreativeRow, RowConfig (+ RowArticleSelection, RowPresetConfig), CampaignBuildItem
...
- **Canvas wizard (per-row setup flow):** `WizardShell` renders in three modes: `canvas` (`CampaignCanvas` React Flow), `review` (`ReviewAndPost`), `done` (success screen). `CampaignCanvas` is loaded via `next/dynamic` with `ssr: false`. The canvas uses `useCanvasStore` (Zustand) to track `creativeRows: CreativeRow[]` (each row holds an ordered list of `groupIds`), `creativeGroups: CreativeGroup[]` (each group holds 1–5 asset IDs), `rowConfigs: RowConfig[]` (one per row — holds `feedProviderIds[]`, `articles[]` with headline/CTA/RAC, `adAccountIds[]`, `presets[]` with duplication count), and `nodePositions`. **There are no edges and no provider/article/account/preset nodes** — every row is fully self-contained. `buildCampaignMatrix()` iterates `rowConfigs` → for each config it looks up its row's `groupIds` → each group emits one `CampaignBuildItem` per `(article × preset × adAccount × duplicationIndex)` combination, with `creativeIds: string[]`. On launch, `WizardShell` loads all assets for `item.creativeIds`, calls `synthesizeCampaign()`, then `runSubmission()`.

  **Creative row concept:** A `CreativeRow` is the top-level canvas unit — one row node containing a row of portrait cards (creative slots) and an inline 4-step config panel beneath them. Each card slot in the row = one `CreativeGroup` = one ad set. Within a group, multiple creatives = multiple ads in that ad set. Node ID prefix: `row-{id}`. Cards render left-to-right; `groupIds` is prepend-on-add so `groupIds[0]` = newest = leftmost, `groupIds[last]` = oldest = rightmost. "+" button above the rightmost card adds a new slot (prepends). Each time a slot is prepended the node's x position is decreased by `ROW_CARD_STEP = 172` (160 + 12) to keep the rightmost card anchored at the same canvas position. A slot with 2–5 creatives shows a clickable **×N badge** that opens an inline overlay revealing mini-thumbnails of all creatives; each mini has a ✕ to remove that creative. "↓ New row" and "⧉ Duplicate" actions appear on row hover below the row. Max 8 slots per row.

  **Per-row config panel (inside `CreativeGroupNode`):** Below the card row, every row carries a self-contained 4-step config panel:
  1. **Feed** — always visible. "+ Add" opens a feed-provider checklist modal. Pills show a colored dot per provider.
  2. **Articles** — visible once ≥1 provider is selected. "+ Add" opens an article checklist filtered to the row's selected providers. Each article pill is clickable to expand an **inline headline + CTA editor** (Headline is a `<select>` populated from `article.allowedHeadlines` — also stores the selected `rac`; CTA is a `<select>` over 10 standard CTAs).
  3. **Accounts** — visible once ≥1 article is selected. "+ Add" opens an ad-account checklist (filtered to non-hidden accounts).
  4. **Preset** — visible once ≥1 account is selected. "+ Add" opens a preset checklist filtered to the row's selected providers (or unbound presets). Each preset pill has an `×N` duplication badge — clicking it converts to an inline number input (1–10).

  Removing a provider pill cascades within the row — articles tied to that provider are removed from `config.articles`. Pickers use the same dark centered-modal pattern.

  **`CampaignCanvas.tsx` design decisions:**
  - **Single node type** — `NODE_TYPES = { group: CreativeGroupNode }`. No edges (`edges={[]}`). No `onConnect`/`onEdgesDelete`.
  - **Empty-state button** — `openAddCreative` creates a new row then opens SiloBrowser. Canvas tracks two Silo targets: `targetRowId` (add new slot to row) and `targetGroupId` (add creative to existing slot). On select, routes to `addGroupToRow` or `addCreativeToGroup` accordingly. Empty row is removed on Silo cancel.
  - **Row node `data`** — receives `providers`, `articles`, `accounts: {id, name}[]`, `presets`, and `providerColorMap` so the row's inline pickers render without re-fetching.
  - **Auto-align** — dagre LR placeholder still uses `computeAutoLayout` but only rows are laid out (single column).
  ...
  **React Flow render-loop hazards (React error #185):** Three pitfalls that cause an infinite `setNodes` loop:
  ...
  The single visibility memo (`visibleAccounts`) is wrapped in `useMemo`. The previous provider/article/preset visibility memos were removed along with their node types.

  **`useCanvasStore` per-row actions:** `addProviderToRow`, `removeProviderFromRow` (cascade-removes that provider's articles from the row), `toggleArticleInRow` (default headline pre-fills from `article.defaultHeadlineIndex` if set; default CTA `"MORE"`), `setRowArticleContent` (headline / CTA / RAC), `toggleAdAccountInRow`, `togglePresetInRow` (default duplications: 1), `setRowPresetDuplications` (clamped 1–10). `addRow()` pushes an empty `RowConfig`; `removeRow(rowId)` filters out the row's config; `duplicateRow(rowId)` deep-copies the source row's config to the new row.

  **Canvas visual rules:**
  - **Provider colors** — assigned from `PROVIDER_COLORS` array indexed by sort-order of `createdAt` (stable; not array position). Colors propagate to the dot in provider pills and to each card's left accent stripe.
  - **CreativeGroupNode (row node)** — horizontal flex row of portrait cards (`160×285px` each, `9/16` aspect) followed by a config panel (`min-width: 420px`, dark rounded card). Left accent stripe per card shows connected provider color(s). Node outer div has `style: { background: "transparent", border: "none", padding: 0 }` to override the React Flow node wrapper background. `groupIds` is rendered without reversal; `groupIds[0]` (newest) is leftmost, `groupIds[last]` (oldest) is rightmost. Each prepend subtracts `ROW_CARD_STEP = 172` from the node's stored x so the rightmost card stays anchored.
  - **Config panel sections** — labels (`Feed`, `Articles`, `Accounts`, `Preset`) fixed in a 14-char gutter; pills wrap; "+ Add" buttons are dashed-border circles. Inline headline/CTA editor sits in an indented sub-card under the Articles row.
