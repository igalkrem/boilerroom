...
- **URL macro system:** Macros split by resolution stage:
  ...
  | `{{organization_id}}` | Snapchat org ID | Snapchat native — substituted at click time (no longer stored in provider settings) |
...
- **Feed Providers (v3):** Full sell-side provider management. `FeedProvider` type lives in `src/types/feed-provider.ts` (not `article.ts`). Key fields:
  - `snapConfig` — `allowedAdAccountIds[]`, `allowedPixelIds[]`, `campaignNamingTemplate?: NamingSegment[]` (Snap-specific; stored per-traffic-source — when Facebook is added it gets its own field)
