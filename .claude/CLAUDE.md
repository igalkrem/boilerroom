...
- **Database:** Neon Postgres via `@vercel/postgres` (`POSTGRES_URL` env var) — reporting cache (3 tables: `snapchat_ad_squad_stats`, `kingsroad_report`, `report_sync_log`) + channel lifecycle (`feed_provider_channels`) + cron token storage (`user_snapchat_tokens`). Migrations run automatically on first call to either `/api/reporting/sync` or `/api/reporting/combined` via `runMigrations()` in `src/lib/db/index.ts`. **Note:** `@vercel/postgres` is deprecated upstream — migrate to `@neondatabase/serverless` when convenient.
...
- `placement_v2.config` accepts `"AUTOMATIC"`, `"CUSTOM"`, and `"CONTENT"` (Stories/Publisher Stories — not in public spec but accepted by the API). `placement_v2` must NOT appear in PUT `/adsquads/{id}` body — squads created with it return E2025 on update.
