...
export function SyncStatusBar({
  ...
  return (
    <div className="flex items-center justify-between mb-4">
      ...
      {/* Force Refresh — escape hatch only; intentionally subtle */}
      <button
        onClick={onForceRefresh}
        disabled={syncing || loading}
        title="Force Refresh — re-fetches all sources regardless of cache"
        aria-label="Force Refresh"
        className="text-gray-600 hover:text-gray-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors p-1 rounded"
      >
        {syncing ? (
          <Spinner />
        ) : (
